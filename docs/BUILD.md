...
`tools/TestUpgrade.ps1` 在本版发布时验证 3.5.7 → 3.5.8。当前验证记录位于本地忽略目录 `artifacts/release-3.5.8/`，旧发布产物归档到 `archive/pre-3.5.8/`。

## 3.5.9 桌面菜单与窗口恢复

- 猫猫与托盘共用菜单在打开时仅激活菜单，不激活猫猫或主窗口。点击其他软件或按 Esc 可关闭菜单；同一次外部点击仍送达目标软件。托盘菜单继续等右键释放处理结束后再打开，避免一闪而过。
- 右键“添加任务”在菜单关闭后直接打开独立编辑窗，主页显隐、位置、历史／待办与列表／树形保持不变。重复点击恢复已有草稿，取消草稿不改变任务数据。
- 猫猫、托盘与“展开任务表”只恢复窗口，不再重置历史／待办、视图或树形缩放与位置。

最终验证：93 项业务检查、673 项 UI 检查通过（225% 缩放）。包括真实鼠标点击、窗口焦点与位置、编辑冲突、任务完成／恢复反馈、长标题、固定操作栏、滚动端点、树形缩放、Markdown 悬停、输入隔离、同屏全屏收起与恢复。程序选择器专项通过，识别 235 个应用，其中 32 个来自 Microsoft Store；搜索、添加、取消勾选反馈与关闭窗口联动正常。已人工查看长标题、树形画布、设置底部及程序选择器截图。游戏兼容验证使用独立进程全屏测试窗口；未直接运行英雄联盟或 WoW，跨屏规则使用不同分辨率及负坐标边界验证。

3.5.8 → 3.5.9 隔离覆盖升级通过，自启动与桌面快捷方式默认勾选；3 个已登记任务／配置文件 SHA256 前后一致，用户真实安装登记未改变。独立卸载验证确认清理选项默认勾选、取消保留程序文件、静默卸载保留无关文件。便携包 14 个文件与发布目录匹配，程序／安装器版本为 3.5.9.0，SHA256 清单核对通过。

`tools/TestUpgrade.ps1` 默认验证 3.5.8 → 3.5.9。验证日志及界面截图保存在本地忽略目录 `artifacts/release-3.5.9/`；旧构建与调试产物归档到 `archive/pre-3.5.9/`。README 仅使用独立演示数据截图。
