...
try {
    ...
    $package = Join-Path $projectRoot 'dist/TinyTodo-Windows-3.5.6'
    ...
    $zipPath = Join-Path $projectRoot 'dist/TinyTodo-3.5.6-Portable.zip'
    Compress-Archive -LiteralPath $package -DestinationPath $zipPath -Force
    $outputs = @((Join-Path $projectRoot 'dist/TinyTodo-3.5.6-Setup.exe'),$zipPath)
    ...
    Write-Output 'Ready: dist/TinyTodo-3.5.6-Setup.exe and dist/TinyTodo-3.5.6-Portable.zip'
