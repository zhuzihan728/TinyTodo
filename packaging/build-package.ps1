...
try {
    ...
    $package = Join-Path $projectRoot 'dist/TinyTodo-Windows-3.5.3'
    ...
    $assetNames = @('app.ico','app-icon.png','floating-icon.png','floating-icon.gif','WenYuanRoundedSC-Regular.ttf','WenYuanRoundedSC-Bold.ttf','Font-coverage.txt','Font-license.txt','Font-source-notice.txt')
    ...
    $zipPath = Join-Path $projectRoot 'dist/TinyTodo-3.5.3-Portable.zip'
    Compress-Archive -LiteralPath $package -DestinationPath $zipPath -Force
    $outputs = @((Join-Path $projectRoot 'dist/TinyTodo-3.5.3-Setup.exe'),$zipPath)
    ...
    Write-Output 'Ready: dist/TinyTodo-3.5.3-Setup.exe and dist/TinyTodo-3.5.3-Portable.zip'
